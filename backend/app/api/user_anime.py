...
from app.services.mal_api import MALApiService, MALApiError
...
async def get_user_anime_list(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Failed to fetch anime list: {str(e)}"
        )
...
async def add_anime_manually(
    ...
    try:
        existing_anime_records = db.get_records("anime", {"name": anime_request.anime_name})
        ...
        if existing_anime_records:
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    anime_model = await fetch_and_insert_anime_data(session, anime_request.anime_url)
                    if anime_model and hasattr(anime_model, 'id') and anime_model.id:
                        anime = db.get_record_by_id("anime", anime_model.id)
            except Exception as e:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f"Failed to scrape anime from URL: {str(e)}"
                )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to add anime: {str(e)}"
        )
    ...
    try:
        ...
        return {
            ...
            "user_anime": {
                ...
                "is_synced_from_mal": updated_user_anime.get("is_synced_from_mal", False)
            }
        }
    except HTTPException:
        raise
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to update anime: {str(e)}"
        )
...
async def remove_anime_from_list(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to remove anime: {str(e)}"
        )
