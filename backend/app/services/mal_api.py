...
class MALApiService:
    ...
    async def get_user_access_token(self, user_id: int) -> str:
        ...
        token_expires_at = datetime.fromisoformat(user["token_expires_at"].replace("Z", "+00:00"))
    ...
    async def fetch_user_anime_list(self, user_id: int, limit: int = 1000) -> List[Dict[str, Any]]:
        ...
        async with httpx.AsyncClient() as client:
            while True:
                ...
                params = {
                    "fields": "list_status,num_episodes,start_date,end_date,synopsis,mean,rank,popularity,genres,media_type,status",
                    "limit": min(limit - len(anime_list), 1000),
                    "offset": offset
                }
                ...
                if len(anime_list) >= limit or len(batch_anime) < 1000:
                    break
        
        return anime_list
    
    async def sync_user_anime_list(self, user_id: int, auto_download: bool = True) -> Dict[str, int]:
        ...
        if auto_download:
            try:
                downloaded_count = await AnimeDownloadService.auto_download_missing_anime(mal_anime_list)
            except Exception as e:
                logger.warning(f"⚠️ Auto-download failed: {str(e)}")
        ...
        for mal_anime_data in mal_anime_list:
            try:
                ...
                existing_user_anime = db.get_records("user_anime", {
                    "user_id": user_id, 
                    "anime_id": anime["id"]
                })
                ...
                if existing_user_anime:
                    db.update_record("user_anime", existing_user_anime[0]["id"], user_anime_data)
                    updated_count += 1
                else:
                    user_anime_data["created_at"] = datetime.now(timezone.utc).isoformat()
                    db.insert_record("user_anime", user_anime_data)
                    created_count += 1
                    
            except Exception as e:
                logger.error(f"❌ Error syncing anime {anime_title}: {str(e)}")
                continue
        
        logger.info(f"🎯 Sync completed: {created_count} created, {updated_count} updated, {downloaded_count} downloaded")
